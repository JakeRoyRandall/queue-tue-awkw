...
END {
    ...
    if (summary == 1 && !failed) {
        if (max_wait != "") { if (utilization == 1) print "summary_lane\tcustomers\tleft_customers\ttotal_service_min\tmean_wait_min\tmax_wait_min\tfinish_min\tbusy_span_min\tidle_min\tutilization_percent"; else print "summary_lane\tcustomers\tleft_customers\ttotal_service_min\tmean_wait_min\tmax_wait_min\tfinish_min" }
